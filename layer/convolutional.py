...
class Convolutional(NeuralLayer):
    ...
    def feed_forward(self, x):
        ...
        z = np.array([scisig.fftconvolve(x, self.Wxz[k], mode="valid")
    ...
    def back_prop(self, delta):
        ...
        x_flipped = self.training_vars["x"][::-1,::-1,::-1]
        for k in range(self.num_patterns):
            self.training_vars["dL_dWxz"][k] \
                += scisig.fftconvolve(x_flipped, dL_dz[k].reshape(1, self.z_height, self.z_width), mode="valid")
            self.training_vars["dL_dbz"][k] += np.sum(dL_dz[k])
            w_k_flipped = self.Wxz[k][::, ::-1, ::-1]
            for r in range(self.x_depth):
                dL_dx[r] += scisig.fftconvolve(dL_dz[k], w_k_flipped[r], mode="full")
        # print(np.sum(np.abs(dL_dx)))
        return dL_dx
